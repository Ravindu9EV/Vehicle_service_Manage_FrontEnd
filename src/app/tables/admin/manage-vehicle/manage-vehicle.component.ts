...
loadVehicles(){
  this.http.get<Vehicle[]>("http://localhost:8080/vehicle/get-all").subscribe((data)=>{
    console.log("hi");
...
console.log(id);
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      this.http.delete(`http://localhost:8080/vehicle/delete-by-id/filter?id=${Number.parseInt(id.toString())}`).subscribe(data=>{
        if(data){
          console.log(data);
          this.loadVehicles()
